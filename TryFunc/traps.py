# Almost Ok, still need to convert into functional, perhaps we should make high-order functions here
import rectangle
import abc
import config
import pygame
...
class Trap(rectangle.Rectangle):
    ...
    ###############   shotEnemy replaced
    def shotEnemies2(self, listOfEnemies):
        if not(is_empty(listOfEnemies)):
            if(self.collide(first(listOfEnemies))):
                self.shot(first(listOfEnemies))
                self.resetReloadTime()
            else:
                self.shotEnemies2(rest(listOfEnemies))

    def shotEnemies(self, enemies, towerDefense):
        if self._reloadTime <= 0:
            self.shotEnemies2(enemies)
###############
